...
For more information, consult from the website[^website], and __References__ Section.
...
```
As can be seen from the summary of the 8 random samples, there are a lot of missing values as well as some "other" variables. The following plot show the extent of these missing values, in most cases entire variables have missing data.
...
Now the data has been cleaned, it must be pre-processed to ensure the best predictors are selected to train the model. This process involves finding correlated  predictors and removing them. This process ensures that any __redundant__ (correlated by __99%__) predictors are removed.   
...
### Model Fitting
To assess the accuracy of how proposed model will generalize to the independent testing data set (__cross-validation__), the data is partitioned into a training subset and validation subset, using a 60/40 split.
...
```
The __Random Forest__ model was selected to predict the manner in which 6 participants performed the exercises for the following reasons:
...
```
The following plot shows the variables that are __estmated__ to have the most impact as predictors.
...
```
The model is then applied to the validation data set.
...
```
As can be seen from the above Confusion Matrix, the overall accuracy of the model on the validation data set is __`r confusionMatrix(Predict, valSet$classe)$overall[[1]]`__. This means that when the model is applied to the independent testing set, there is an __out of sample__ error rate of __`r 1 - confusionMatrix(Predict, valSet$classe)$overall[[1]]`__.
...
### Conclusion
When applied to the 20 test cases, the Random Forest Machine learning algorithm correctly predicted the manner in which the exercises were preformed.
