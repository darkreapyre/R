...
The plot in [Appendix A: Box Plot][] shows the distribution of the transmission types for the various vehicles and their respective MPG. As can be seen, the vehicles with a Manual transmission have an average of __`r mean_mpg[[2]]`__ MPG, while the vehicles with an Automatic transmission have an average of __`r mean_mpg[[1]]`__ MPG. It also seems that the vehicles with __4__ gears (whether Automatic or Manual) are more economical. Since there are other variables that can influence this conclusion (See [Appendix B: Pair-wise Plot][] for other highly correlated variables), the rest of this report will quantify these more. Since there are no missing values or outliers within the data set, however there are a number of variables that have a number of categories. So to prepare the data for a better analysis, we factorize the variables into their individual categories and provide better names for the type of transmission. 

## Regression Analysis and Model Selection
Now that the data is neatened and factorized, we can run an initial regression analysis to get an idea of which of the variables have an effect on fuel economy. To start the process of finding the best model we fit the linear regression model using all the variables as predictors. 
...
* The best model determined using automatic methods produces as adjusted $R^2$ of __0.84__.

[Appendix C: Residual Plot][]
...
****
## Appendix A: Box Plot
```{r boxplot, echo = FALSE, fig.height = 4, fig.width = 5}
#Box Plot
data(mtcars)
par(mfrow = c(1, 2))
...
## Appendix B: Pair-wise Plot 
```{r pair, echo = FALSE, fig.height = 8, fig.width = 7}
#Pair-wise plot
...
## Appendix C: Residual Plot
```{r residual, echo = FALSE, fig.height = 8}
#Rsediual Plot
par(mfrow = c(2, 2))
plot(best_fit)
```
