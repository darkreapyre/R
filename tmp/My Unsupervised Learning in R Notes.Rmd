...
For __Principle Components__, we will use a dataset already in __R__ called the `USArrests` data. This data set contains statistics, in arrests per 100,000 residents for assault, murder, and rape in each of the 50 US states in 1973. Also given is the percent of the population living in urban areas. The Data is provided as a data frame with __`r dim(USArrests)[1]`__ observations (for each of the States) and __`r length(USArrests)`__ variables for __`r names(USArrests)`__.
...
From the data above, we see tha the __means__ and the __variances__ are very different. __Principle Components__ are all about __variance__ (so the __means__ don't really play a role), so the __variance__ of the individual vriables are most important. This is due to the fact that we are looking for linear combinations that maximize the variance. So if there is a single variable that dominates with respect to variance it will be the principle component. this can be seen from the output of the variance above,`Assault` has the highest degree of variance over the other variables, so it will dominate the principle components and therefore we don't get anything meaningful from the data. This is basically because each of the different variables is measured in different units.  
...
3. The summeries for each of the clusters:
...
The plot above shows the cluster assignments found by the __K-Means__ Algorithm. __Note__ the black point, which actually belonged to the black cluster from the original data, BUT was assigned to the green cluster. Also there's a blue point that originally belonged to blue cluster, but has now been assigned to the black cluster. Please see __Appendix A__ which highlights discrepencies when generating the colos for the plots and how to redolve these discrepenacies.

# Hierarchical Clustering

## Overview

To demonstrate Hierarchical Clustering, which is another form of clustering as it works off a distance matrix, we wil use the same "fake" data as before. 

## Generating the Hiurarchical Cluster

To build the cluster, we will compute the distance of __X__ using the `dist()` function, which create a __100__ by __100__ pair-wise distance matrix from the "fake" data and then call `hclust()`, which is the tool for doing Heirachical Clustering. 
...
#Generate the modelwith the smallest distance between clusters
...
As can be seen from the plot above, the __Cluster Dendrogram__ is very difference when using the smallest distance between clusters. 
...
__Note__ that the coloring scheme shown is not the sames as the origional color scheme used when we generated the data. The above coloring scheme depcists the cluster assignments found by the K-means clustering algorithm and the difference is distictly visible When we overlay the above plot on the origional data (with the original clusters):
...
It is clearly evedent that we have a color mismatch. This is becasue we named the clusters 1 to 4 in some order. The K-Means clustering algorithm found four clusters and it named them 1 to 4, BUT the ordering is arbitrary, so it picked a different order. So to produce the final plot showing the results of the K-Means Algorithm with the origional clusters we created, we just reassign the colors as follows:
