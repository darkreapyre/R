...
* Consider predicting $Y$ at a value of $X$ in an inferential way. For example:
...
$$
* But if we want the prediction interval - __something that incorporates the uncertainty associated with the interval__ - a standard error is needed.
* Knowledge of the distribution of the prediction estimate is also required to create the prefdiction interval.

* There's a subtle distinction between intervals for the regression line at point $x_0$ and the prediction of what a $y$ would be at point $x_0$. (See the plot below). 
* Line at $x_0$ standard error, $\hat \sigma\sqrt{\frac{1}{n} +  \frac{(x_0 - \bar X)^2}{\sum_{i=1}^n (X_i - \bar X)^2}}$ 
* Prediction interval standard error at $x_0$, $\hat \sigma\sqrt{1 + \frac{1}{n} + \frac{(x_0 - \bar X)^2}{\sum_{i=1}^n (X_i - \bar X)^2}}$ 
* The only thing that different really between the two equations is the $1$ to produce a wider interval for the prediction interval.

```{r plot1, edcho = TRUE}
#The following code plots the intervals using the equation
#Plot the X and Y
plot(x, y, frame = FALSE, xlab = "Carat", ylab = "(SING) Dollars", pch = 21,
     col = "black", bg = "lightblue", cex = 2)

#Add the fitted regression line
abline(fit, lwd = 2, col = "red")

#Create the X values that we wish to predict at
xVals <- seq(min(x), max(x), by = .01)

#Create the y values for the predictions
yVals <- beta0 + beta1 * xVals

#Create the two standard errors per the equations above
se1 <- sigma * sqrt(1 / n + (xVals - mean(x))^2/(sum((x - mean(x))^2)))
se2 <- sigma * sqrt(1 + 1 / n + (xVals - mean(x))^2/(sum((x - mean(x))^2)))

#Plot the lines for the prediction intervals
lines(xVals, yVals + 2 * se1, col = "blue")
lines(xVals, yVals - 2 * se1, col = "blue")
lines(xVals, yVals + 2 * se2, col = "green")
lines(xVals, yVals - 2 * se1, col = "green")

#The following code implements the intervals using R funcitons
...
## Key Points on Prediction Intervals

* Both intervals have varying widths. 
  * Least width at the mean of the $X$'s. 
* We are quite confident in the regression line, so that interval is very narrow. 
  * Or, if we knew $\beta_0$ and $\beta_1$ this interval would have zero width. 
* The prediction interval must incorporate the variabilibity in the data around the line. 
  * So, even if we knew $\beta_0$ and $\beta_1$ this interval would still have width. 

\newpage

# Multivariable Regression Analysis

__Linear Regression is NOT equiped to handle more than one predictor__

How can one generalize Simple Linear Regression to incoporate lots of regressors for the purpose of prediction? 
...
* The sample estimate of any __population__ parameter puts a "hat" on the parameter. So if __$\beta$__ is the parameter then __$\hat \beta$__ is the estimate of that true parameter value. 
* A Statistic and a Parameter are very similar. A Statistic describes a __Sample__, while a Parameter describes the __Population__.
...
__We want variability (spread) in the $X$'s because it decreases the varbiability on the slope ($\hat \beta_1$)__
